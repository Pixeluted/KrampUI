...
export class DataManager {
    ...
    public static async initialize() {
        if (!await FileSystemService.exists("")) {
            ...
            if (!results.success) {
                WindowManager.showFatalErrorPopup(`Failed to create application data directory. Error: ${results.error}`);
            }
        }
        ...
        if (!await FileSystemService.exists("settings")) {
            ...
            if (!results.success) {
                WindowManager.showFatalErrorPopup(`Failed to create settings directory. Error: ${results.error}`);
            }
        }
